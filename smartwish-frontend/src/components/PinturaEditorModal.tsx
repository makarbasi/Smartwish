  ...
  const handleLoad = (res: unknown) => {
    ...
    const injectAIPromptIntoRetouch = () => {
      const modifyRetouchTab = () => {
        ...
        const isRetouchTabActive =
          retouchTab.getAttribute("aria-selected") === "true";
        ...
        const retouchFooter =
          retouchPanel.querySelector(".PinturaUtilFooter") ||
            ...
            <div class="PinturaTabPanels PinturaControlPanels">
              <div class="PinturaTabPanel PinturaControlPanel" aria-hidden="false" data-inert="false" role="tabpanel">
                ...
                <style>
                  ...
                  .ai-style-button{flex:0 0 auto;border:1px solid rgba(0,0,0,0.08);border-radius:12px;padding:8px;display:inline-flex;flex-direction:column;align-items:center;justify-content:center;gap:0.35rem;background:#fff;width:96px;height:104px;box-sizing:border-box;cursor:pointer;transition:all 0.2s ease}
                  .ai-style-button:hover{background:#f9f9f9}
                  /* highlight only the image (thumb) when selected */
                  .ai-style-button.selected{background:#fafafa;}
                  .ai-style-button .ai-style-thumb{position:relative;border:2px solid transparent;}
                  .ai-style-button.selected .ai-style-thumb{border-color:#4f46e5;box-shadow:0 0 0 2px rgba(79,70,229,0.25);border-radius:12px;}
                  .ai-style-thumb{width:56px;height:56px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:14px;transition:transform 0.2s ease, box-shadow 0.2s ease, border-color 0.2s ease}
                  ...
                  .ai-spinner{width:18px;height:18px;border:2px solid rgba(255,255,255,0.4);border-top-color:#fff;border-radius:50%;animation:ai-spin 0.8s linear infinite}
                </style>
        ...
        // Add event listeners
        const sendButton = aiInterface.querySelector(
          "#ai-send-button"
        ) as HTMLButtonElement | null;
        const promptInput = aiInterface.querySelector(
          "#ai-prompt-input"
        ) as HTMLInputElement | null;
        const styleButtons = Array.from(
          aiInterface.querySelectorAll(".ai-style-button")
        ) as HTMLButtonElement[];
        const styleContainer = aiInterface.querySelector(
          "#ai-style-container"
        ) as HTMLElement | null;
          ...
          const activate = async () => {
            ...
            if (styleContainer) {
              ...
              const offset =
                ...
                (contRect.left + contRect.right) / 2;
        ...
        sendButton?.addEventListener("click", async () => {
          ...
          try {
            ...
            if (!imageBlob)
              throw new Error("Could not obtain edited image from the editor");
            ...
            tab.addEventListener("click", () => {
              ...
              if (retouchPanel) {
                ...
                if (
                  ...
                  const originalHtml =
                    retouchFooter.getAttribute("data-original-html");
                  if (originalHtml) {
