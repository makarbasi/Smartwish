...
type Item = { href: string; label: string; icon: React.ComponentType<Record<string, unknown>> }
...
function PageNavigator() {
    ...
    const handlePrevPage = () => {
        if (currentPage > 0) {
            ...
            window.dispatchEvent(new CustomEvent('pageNavigation', { detail: { action: 'prev', page: currentPage - 1 } }))
        }
    }
    ...
    const handleNextPage = () => {
        if (currentPage < 4) {
            setCurrentPage(currentPage + 1)
            // Emit custom event for page navigation
            window.dispatchEvent(new CustomEvent('pageNavigation', { detail: { action: 'next', page: currentPage + 1 } }))
        }
    }
    ...
    const goToPage = (pageIndex: number) => {
        ...
        // Emit custom event for page navigation
        window.dispatchEvent(new CustomEvent('pageNavigation', { detail: { action: 'goto', page: pageIndex } }))
    }
        ...
        <>
            ...
            >
                ...
                <div className="mt-auto w-full px-1 pb-1" ref={popoverRef}>
                    <button onClick={() => setProfileOpen((v) => !v)} className="flex w-full items-center justify-between rounded-md px-1.5 py-1.5">
                        <span className="sr-only">Open profile</span>
                        <Image src="https://i.pravatar.cc/80?img=12" alt="" width={36} height={36} className="h-9 w-9 rounded-full ring-1 ring-gray-200" />
                    </button>
                    {profileOpen && (
                        <>
                            <div className="absolute bottom-12 left-12 z-30 w-72 rounded-xl border border-gray-200 bg-white text-sm shadow-xl">
                                <div className="p-3">
                                    <div className="text-[10px] font-semibold uppercase tracking-wide text-gray-500">Accounts</div>
                                    <div className="mt-2 flex items-center rounded-lg bg-gray-50 p-2 ring-1 ring-gray-200">
                                        <div className="flex items-center gap-3">
                                            <Image src="https://i.pravatar.cc/80?img=12" alt="" width={36} height={36} className="h-9 w-9 rounded-full ring-1 ring-gray-200" />
                                            <div>
                                                <div className="text-sm font-semibold text-gray-900">Abubakar Tariq</div>
                                                <div className="text-xs text-gray-500">abubakar72@gmail.com</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="border-t border-gray-200 p-1">
                                    <Link href="/settings" onClick={() => setProfileOpen(false)} className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-gray-700 hover:bg-gray-50">
                                        <Cog6ToothIcon className="h-5 w-5 text-gray-400" /> Settings
                                    </Link>
                                    <a href="#" className="flex items-center gap-3 rounded-md px-2 py-2 text-gray-700 hover:bg-gray-50">
                                    ...
                                    <a href="#" className="flex items-center gap-3 rounded-md px-2 py-2 text-gray-700 hover:bg-gray-50">
                                        <CurrencyDollarIcon className="h-5 w-5 text-gray-400" /> Plans and pricing
                                    </a>
                                    <a href="#" className="flex items-center gap-3 rounded-md px-2 py-2 text-gray-700 hover:bg-gray-50">
                                ...
                                <div className="border-t border-gray-200 p-2">
                                    <a href="/sign-in" className="flex items-center gap-3 rounded-md bg-red-50 px-2 py-2 font-medium text-red-600 hover:bg-red-100">
                                        <ArrowRightStartOnRectangleIcon className="h-5 w-5" /> Log out
                                    </a>
                                </div>
                            </div>
                        </>
