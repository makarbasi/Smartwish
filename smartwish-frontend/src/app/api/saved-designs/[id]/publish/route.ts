...
export async function POST(
  ...
  try {
    ...
    console.log('Publish API - API_BASE_URL:', API_BASE_URL);
    console.log('Publish API - Full URL:', `${API_BASE_URL}/saved-designs/${id}/publish`);

    const response = await fetch(`${API_BASE_URL}/saved-designs/${id}/publish`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { error: 'Failed to publish design', details: errorMessage },
      { status: 500 }
    );
  }
}