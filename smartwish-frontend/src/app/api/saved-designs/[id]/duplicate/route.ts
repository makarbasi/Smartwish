...
export async function POST(
  ...
  try {
    ...
    console.log(
      "Duplicate API - Session user object:",
      JSON.stringify(session.user, null, 2)
    );
    console.log("Duplicate API - Access Token exists:", !!accessToken);
    console.log(
      "Duplicate API - Access Token (first 20 chars):",
    ...
    const response = await fetch(
      `${API_BASE_URL}/saved-designs/${id}/duplicate`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error(
        "Duplicate API - Error response headers:",
      ...
      throw new Error(
        `Failed to duplicate design: ${response.status} - ${errorText}`
      );
    }
    ...
    try {
      ...
      console.error(
        "Duplicate API - Response text that failed to parse:",
        responseText
      );
