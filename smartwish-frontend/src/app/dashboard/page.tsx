...
export default function Dashboard() {
  ...
  const hasAccessToken = Boolean(
    session?.user &&
      (session.user as unknown as Record<string, unknown>)["access_token"]
  );
  ...
  const isPreventableEvent = (
    ...
    return (
      !!ev &&
      typeof (ev as Record<string, unknown>)["preventDefault"] === "function"
    );
  };
  ...
  const handleSignOut = async (
    ...
    try {
      ...
      console.log(
        "[Dashboard] signOut triggered event:",
        (e as unknown as Record<string, unknown>)["type"] ?? "none"
      );
  ...
  useEffect(() => {
    console.log(
      "[Dashboard] mounted - setting up global touch/pointer listeners"
    );
    const onPointerDown = (ev: PointerEvent) => {
      console.log("[Dashboard] global pointerdown", {
        type: ev.type,
        x: ev.clientX,
        y: ev.clientY,
        target: (ev.target as Element)?.tagName,
      });
    };
    const onTouchEnd = (ev: TouchEvent) => {
      const t = ev.changedTouches && ev.changedTouches[0];
      console.log("[Dashboard] global touchend", {
        type: ev.type,
        x: t?.clientX,
        y: t?.clientY,
        target: (ev.target as Element)?.tagName,
          ...
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                onClickCapture={(e) =>
                  console.log("[Dashboard] onClickCapture", e.type)
                }
