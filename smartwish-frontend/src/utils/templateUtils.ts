...
function validateNoBlobUrls(images: string[]): void {
  ...
  if (blobUrls.length > 0) {
    console.error(
      "❌ CRITICAL: Blob URLs detected in template save operation:",
      blobUrls
    );
    throw new Error(
      `Cannot save template with blob URLs. Found ${blobUrls.length} blob URLs that must be converted to Supabase URLs first.`
    );
  }
}
...
async function ensureSupabaseUrls(images: string[]): Promise<string[]> {
  ...
  if (blobUrls.length === 0) {
    console.log(
      "✅ No blob URLs detected, all images are already Supabase URLs"
    );
    return images;
  }
