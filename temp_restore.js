...
async function downloadImage(url, savePath) {
  console.log(`  ≡ƒôÑ Downloading: ${url}`);
...
async function printPdf(pdfPath, printerName) {
  console.log(`  ≡ƒû¿∩╕Å Printing to: ${printerName}`);
  console.log(`  ≡ƒôä Settings: Letter, Landscape, Duplex (flip short edge), Color`);
  ...
  try {
    ...
    console.log('  Γ£à Print job sent successfully (duplex: flip on short edge)!');
  } catch (err) {
    console.warn('  ΓÜá∩╕Å Print with duplex options failed:', err.message);
    console.warn('  ≡ƒô¥ Trying basic print...');
    // Fallback: try basic print (duplex should be set in printer defaults)
    await print(pdfPath, { printer: printerName });
    console.log('  Γ£à Print job sent using printer defaults');
    console.log('');
    console.log('  ΓÜá∩╕Å  If not printing two-sided, set duplex in Windows:');
    console.log('     Control Panel ΓåÆ Devices and Printers');
    console.log('     Right-click printer ΓåÆ Printing Preferences');
...
async function processJob(job) {
  ...
  try {
    ...
    console.log('  ≡ƒöº Creating composite images...');
    ...
    console.log(`  Γ£à Job ${job.id} completed successfully!`);

  } catch (error) {
    console.error(`  Γ¥î Job ${job.id} failed:`, error.message);
...
async function updateJobStatus(jobId, status, error = null) {
  try {
    ...
    console.warn(`  ΓÜá∩╕Å Could not update job status: ${err.message}`);
  }
}
...
async function pollForJobs() {
  try {
    ...
    if (pendingJobs.length > 0) {
      console.log(`\n≡ƒô¼ Found ${pendingJobs.length} pending job(s)`);
...
async function listPrinters() {
  console.log('\n≡ƒôï Available Printers:');
  console.log('ΓöÇ'.repeat(50));
...
async function main() {
  ...
  console.log('  ≡ƒû¿∩╕Å  SMARTWISH LOCAL PRINT AGENT');
  console.log('ΓòÉ'.repeat(60));
  ...
  console.log('≡ƒöä Waiting for print jobs...');
