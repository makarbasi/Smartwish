...
/**
 * WebSocket Gateway for real-time screen streaming
 * 
 * Architecture:
 * - Kiosks connect and send binary screen frame data
 * - Admin viewers connect and subscribe to specific kiosk feeds
 * - Gateway relays screen frames from kiosk to all subscribed viewers
 * 
...
export class ScreenGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    constructor(private readonly surveillanceService: SurveillanceService) {
        console.log('[ScreenWS] Gateway initialized');
    }

    afterInit() {
        console.log('[ScreenWS] WebSocket server started at /ws/screen');

        // Heartbeat to detect dead connections
        setInterval(() => {
            this.server?.clients?.forEach((ws: AuthenticatedSocket) => {
                if (ws.isAlive === false) {
                    console.log('[ScreenWS] Terminating dead connection');
                    return ws.terminate();
                }
                ws.isAlive = false;
                ws.ping();
    ...
    handleConnection(client: AuthenticatedSocket, request: IncomingMessage) {
        ...
        console.log('[ScreenWS] New connection from:', request.socket?.remoteAddress);
    ...
    handleDisconnect(client: AuthenticatedSocket) {
        ...
        if (client.isKiosk && client.kioskId) {
            ...
            console.log(`[ScreenWS] Kiosk disconnected: ${client.kioskId}`);
        }
            ...
            if (viewerSet) {
                ...
                if (viewerSet.size === 0) {
                    ...
                    if (kioskSocket?.readyState === WebSocket.OPEN) {
                        console.log(`[ScreenWS] No more viewers for ${kioskId}, telling kiosk to stop screen streaming`);
                        kioskSocket.send(JSON.stringify({ type: 'stop_streaming' }));
                    }
                }
            }
            this.stats.totalViewers--;
            console.log(`[ScreenWS] Viewer disconnected from: ${kioskId} (remaining viewers: ${viewerSet?.size || 0})`);
        }
    }
    ...
    private async handleMessage(client: AuthenticatedSocket, data: Buffer | string) {
        ...
        try {
            ...
            console.log(`[ScreenWS] Received message: event=${event}`);
            ...
            switch (event) {
                ...
                case 'frame':
                    ...
                    break;
                default:
                    console.log(`[ScreenWS] Unknown event: ${event}`);
            }
        } catch (e) {
            // Not JSON - could be binary frame or invalid message
            console.log('[ScreenWS] Non-JSON message received:', e);
        }
    }
    ...
    private async handleKioskAuth(
        ...
        console.log(`[ScreenWS] Auth attempt from kiosk: ${kioskId}`);

        // Validate API key
        const isValid = await this.surveillanceService.validateKioskApiKey(kioskId, apiKey);
        if (!isValid) {
            console.log(`[ScreenWS] Invalid API key for kiosk: ${kioskId}`);
        ...
        console.log(`[ScreenWS] Kiosk authenticated for screen streaming: ${kioskId}`);
        ...
        if (viewerSet && viewerSet.size > 0) {
            console.log(`[ScreenWS] ${viewerSet.size} viewers waiting for ${kioskId}, starting screen stream`);
    ...
    private handleViewerSubscribe(
        ...
        if (!hadViewers && kioskConnected) {
            console.log(`[ScreenWS] First viewer for ${kioskId}, telling kiosk to start screen streaming`);
            kioskSocket!.send(JSON.stringify({ type: 'start_streaming' }));
        }

        console.log(`[ScreenWS] Viewer subscribed to screen: ${kioskId} (kiosk ${kioskConnected ? 'online' : 'offline'}, viewers: ${this.viewers.get(kioskId)!.size})`);
    ...
    private handleFrame(client: AuthenticatedSocket, frameData: Buffer) {
        ...
        if (sentCount > 0) {
            ...
            if (this.stats.framesRelayed % 100 === 0) {
                console.log(`[ScreenWS] Relayed ${this.stats.framesRelayed} screen frames total`);
            }
        }
    }
